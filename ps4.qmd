...
def classify_topic(title):
    ...
    if any(word in t for word in [
        "doctor","hospital","nurse","clinic","medicare","medicaid",
        "patient","pharmacy","provider","physician","therapy",
        "laboratory","lab","telehealth","hospice","home health",
        "ambulance","imaging","medical"
    ]):
        return "Health Care Fraud"

    # Financial Fraud
    elif any(word in t for word in [
        "bank","loan","financial","investment","insurance","billing",
        "tax","money laundering","wire fraud"
    ]):
        return "Financial Fraud"

    # Drug Enforcement
    elif any(word in t for word in [
        "drug","opioid","fentanyl","prescription","controlled substance",
        "trafficking"
    ]):
    ...
    # Bribery / Corruption
        ...
        "bribe","kickback","corruption","conspiracy"
    ]):
...
```{python}
df_2022["topic"] = df_2022["title"].apply(classify_topic)

monthly_topic = (
    df_2022.groupby(["year_month", "topic"])
    .size()
    .reset_index(name="n_actions")
)
